# Script to compile every cheetah template for cosmosgen.py

echo "Compiling Cheetah Templates"

cheetah compile Channel_Screen.tmpl
cheetah compile Channel.tmpl
cheetah compile Command.tmpl
cheetah compile Config_Data_Viewer.tmpl
cheetah compile Config_Server.tmpl
cheetah compile Config_System.tmpl
cheetah compile Config_Tlm_Viewer.tmpl
cheetah compile Data_Viewer.tmpl
cheetah compile Event.tmpl
cheetah compile Partial_Channel.tmpl
cheetah compile Partial_Command.tmpl
cheetah compile Partial_Data_Viewer.tmpl
cheetah compile Partial_Event.tmpl
cheetah compile Ruby_Evr_Dump_Component.tmpl
cheetah compile Ruby_Multi_String_Tlm_Item_Conversion.tmpl
cheetah compile Ruby_Ref_Protocol.tmpl
cheetah compile Server.tmpl
cheetah compile Target.tmpl
