...
__version__ = "1.0"
__email__ = "aaron.doubek-krat@jpl.nasa.gov"
...
class History(object):
    ...
    def add(self, data):
        ...
        @param data: a list of tuples in the form (name, value). The name values will be
    ...
    def __assert_hist_size(self, size, comp, id, filterFunc):
        ...
        '''
        if id: id = id.upper() #Coerce name to uppercase for backward compatibility
    ...
    def __assert_hist(self, value, comp, id, index):
        ...
        The comparison to perform is passed by the function argument 'comp'
        '''
        if id: id = id.upper() # Coerce name to all uppercase for backward compatibility
...
class EventHistory(History):
    ...
    '''
    _telem = cosmos_telem_loader.COSMOSTelemLoader.get_instance()
    _type_name = "event"

    def _name_valid(self, name):
        return self._telem.is_event(name.upper())
...
class TelemetryHistory(History):
    ...
    def _name_valid(self, name):
        return self._telem.is_channel(name.upper())
