...
def main(argv=None):
    ...
    api = GseApi(generated_path=opts.generated_path, server_addr=opts.addr, port=opts.port)
    cmd_list = api.list()
    print "List of commands found:", cmd_list
    time.sleep(1)
    api.send("CMD_NO_OP")
    time.sleep(1)
    api.send("CMD_NO_OP_STRING", args=[3, 'Foo'])
    time.sleep(1)
    print api.send_wait_evr("CMD_TEST_CMD_1", 'OpCodeCompleted', [1, 2.0, 3])
    time.sleep(1)
    print api.send_wait_tlm("CMD_NO_OP", 'CommandsDispatched', timeout = 10, args=None)
