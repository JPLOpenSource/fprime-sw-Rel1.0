...
OpenMCT ships with plugins to use Apache CouchDB to save and persist user-created
...
## Usage
This package requires node.js (> 8.x) and npm (> 5.x) to be installed on your system to build and run (https://nodejs.org/en/).
...
| client/plugins | Custom plugins for the fprime server are located here. Currently only `VueLADTable`.
